      ...
      <el-table :data="roleList" border stripe>
        ...
        <el-table-column label="操作" width="300px">
          <template v-slot:default="scope">
            ...
            <el-button size="small" type="warning" icon="el-icon-setting" @click="showSetRightDialog(scope.row)">
              分配权限
            </el-button>
          </template>
        </el-table-column>
...
function validateEnglishOnly(rule, value, callback) {
...
const state = reactive({
  ...
  addFormRules: {
    value: [
      ...
      { validator: validateEnglishOnly, trigger: "blur" },
    ],
...
function addRole() {
  addFormRef.value.validate(async (valid) => {
    if (!valid) return;
    const { data: result } = await proxy.$http.post("auth/roles", addForm.value);
    if (!checkRequestResult(result, "添加角色失败！")) {
      return;
    }
    proxy.$eMessage.success("添加角色成功！");
...
async function showEditDialog(id) {
  ...
  const { data: result } = await proxy.$http.get("auth/roles/" + id);
...
function editUserInfo() {
  editFormRef.value.validate(async (valid) => {
    ...
    if (!valid) return;
    // 发起修改用户信息的数据请求
    const { data: result } = await proxy.$http.put("auth/roles/" + editForm.value.roleId, {
...
// 根据id删除对应的用户信息
async function removeRoleById(id) {
  // 弹框提示用户是否删除数据
  ...
  const { data: result } = await proxy.$http.delete("auth/roles/" + id);
  if (!checkRequestResult(result, "删除角色失败")) {
    return;
  }
  proxy.$eMessage.success("删除角色成功");
  getRolesList();
}
 ...
 */
async function removeRightById(role, permissionId) {
  // a. 弹框提示用户是否删除数据
  ...
  const { data: result } = await proxy.$http.delete("auth/roles/" + role.id + "/permission/" + permissionId);
  if (!checkRequestResult(result, "删除权限失败！")) {
    return;
  }
  proxy.$eMessage.success("删除权限成功");
...
async function showSetRightDialog(role) {
  ...
  // 获取所有权限数据
  const { data: result } = await proxy.$http.get("auth/permissions/tree");
  if (!checkRequestResult(result, "获取权限数据失败！")) {
    return;
  }
  // 获取到的权限数据保存到permissionInfoList中
  permissionInfoList.value = result.data.infos;
  // 递归获取三级节点的id
...
function getLeafKeys(node, arr) {
  ...
  node.permission_infos.forEach((item) => getLeafKeys(item, arr));
}
 ...
 */
function buildSelectedPermissionTree(infos, selectKeys) {
  let newTree = [];
  for (let node of infos) {
    if (selectKeys.includes(node.id)) {
      ...
      // 递归地对子节点进行同样的操作
      let filteredChildren = buildSelectedPermissionTree(node.children, selectKeys);
...
async function allotRights() {
  ...
  var newPermissionInfos = buildSelectedPermissionTree(permissionInfoList.value, keys);

  const idStr = keys.join(",");
  const { data: result } = await proxy.$http.put(`auth/roles/${roleId.value}`, {
    permission_ids: newPermissionInfos,
  });
  if (!checkRequestResult(result, "分配权限失败！")) {
    return;
  }
  proxy.$eMessage.success("分配权限成功");
  getRolesList();
